...
class CDnetV2_MODEL(nn.Module):
    def __init__(self, block, layers, in_channels=3, out_channels=2, aux=True):
...
def CDnetV2(in_channels=3, out_channels=2):
    model = CDnetV2_MODEL(Bottleneck, [3, 4, 6, 3], 
...
if __name__ == '__main__':
    ...
    model = CDnetV2(in_channels=3, out_channels=4).to(device)
    x = torch.randn(size=(8, 3, 256, 256)).to(device)
    y = model(x, x)
    print(y.shape)
